...
impl VirtualNode
{
    ...
    /// Inserts this node into the retained DOM, as a child of the node specified by `parent`,
    /// if not `None`.
...
/// Updates the node with the specified id in the retained DOM.
pub(super) fn update_node(arena: &mut Arena<RetainedNode>, id: NodeId, vn: VirtualNode)
{
    // we recreate a new retained node if the content type
    // of the nodes do not match
    ...
    if recreate {
        ...
        // update in place
        {
            ...
            if data.class != vn.class {
                // the CSS class has changed, mark styles as dirty
                // they will be reloaded from the stylesheet as soon as they are needed
...
fn update_children(arena: &mut Arena<RetainedNode>, parent: NodeId, mut vdom: Vec<VirtualNode>)
{
    ...
    // point to children of node in retained DOM
    let mut next = arena[parent].first_child();

    // add each element
    'outer: for (vi,vn) in vdom.drain(..).enumerate() {
        if let Some(n) = next {
            // there is a child in the retained DOM at the current position
            // try to update it
            update_node(arena, n, vn);
            next = arena[n].next_sibling();
        } else {
            // there are no more children in the DOM, create new nodes
            vn.into_retained(arena, Some(parent));
        }
    }

    // drop all extraneous nodes in the retained DOM
