...
impl ClipTreeNode
{
    ...
    /// Dumps the node to the standard output.
...
fn hit_test_node(node: &ClipTreeNode, pos: (f32,f32), hits: &mut Vec<NodeId>)
{
    ...
    // if this node was hit, also test its children.
    // Do it anyway if the node doesn't clip its children:
...
impl ClipTree
{
    /// Dumps the tree to the standard output.
    fn dump(&self)
    {
        self.root.dump(0);
    }
    ...
    pub fn hit_test(&self, pos: (f32,f32)) -> Vec<NodeId>
    {
        ...
        // reverse the list of hits, so that the most derived node is first
        // FIXME this may not be consistent with Z-order! needs testing
        hits.reverse();
        //debug!("ClipTree::hit_test {},{} -> {:?}", pos.0, pos.1, hits);
        hits
    }
}

/// Recursive function that builds a clip tree.
/// Also updates the `bounds` field of RetainedNodes.
    ...
    parent_bounds: &Bounds) -> ClipTreeNode
{
    let mut clip_node = {
        ...
        // drop borrow of nodes
    };

    // recurse over children
    let mut next = nodes[id].first_child();
...
fn build_clip_tree(nodes: &mut Arena<RetainedNode>, root: NodeId, bounds: &Bounds) -> ClipTree
{
    ClipTree {
...
const DEBUG_CORNER_SIZE: f32 = 5.0;

/// Draws a rectangle that visualizes the specified bounds on the screen.
fn draw_bounds(
    frame: &nvg::Frame,
    bounds: &Bounds)
{
    ...
    let corners = nvg::Color::new(0.0, 0.0, 1.0, 1.0);
    let sides = nvg::Color::new(0.0, 0.0, 1.0, 1.0);


    // bounds.left -> bounds.left + min(corner size, width)
}
    ...
    style: &Styles)
{
    ...
    //debug!("render rect {},{}:{}x{} radius {} fill ({},{},{},{})", bounds.left, bounds.top, width, height, border_radius, fill_paint.red(), fill_paint.green(), fill_paint.blue(), fill_paint.alpha());
...
impl Renderer {
    ...
    pub fn new(main_window: &GlWindow, events_loop: &EventsLoop, cfg: &Config) -> Renderer
    {
        let debug_bounds = cfg.get::<bool>("ui.renderer.debug_bounds").ok_or(false);
        ...
        // load default font
        // we cannot cache the returned handle, as it borrows the nanovg context:
        // and we can't store both the borrower (font handle) and the borrowed (context)
        // in the same struct (Renderer). Self-referential structs strike again!
    ...
    /// Dumps the internal clip tree of the renderer to the standard output.
    /// Use for debugging purposes.
    pub(super) fn dump_clip_tree(&self) {
        if let Some(ref clip_tree) = self.last_clip_tree {
            clip_tree.dump();
        } else {
            warn!("dump_clip_tree called with no clip tree");
        }
    }
}
